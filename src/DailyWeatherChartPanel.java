...
/**
 * Responsive chart card for seven-day high, low, and precipitation trends.
 */
public class DailyWeatherChartPanel extends SurfacePanel {
    private static final Color GRID = new Color(212, 226, 231);
    private static final Color HIGH = AppTheme.ACCENT;
    private static final Color LOW = AppTheme.OCEAN;
    private static final Color RAIN = new Color(53, 142, 186);
    private static final Color DOT = Color.WHITE;

    private List<DailyForecast> forecasts = List.of();

    /**
     * Creates the daily trend graph.
     */
    ...
    protected void paintComponent(Graphics graphics) {
        ...
        try {
            ...
            drawTitle(g2, left, top, "7 ditët e ardhshme", "Maksimumi, minimumi dhe reshjet");
            ...
            drawTemperatureLine(g2, chartLeft, chartRight, chartTop, chartBottom, minTemp, maxTemp, true);
            drawTemperatureLine(g2, chartLeft, chartRight, chartTop, chartBottom, minTemp, maxTemp, false);
            drawDateLabels(g2, chartLeft, chartRight, chartBottom);
        } finally {
            g2.dispose();
        }
    }
    ...
    private void drawLegend(Graphics2D g2, int right, int top) {
        ...
        int x = right - 210;
        g2.setColor(HIGH);
        g2.fillRoundRect(x, top + 4, 18, 5, 5, 5);
        g2.setColor(AppTheme.MUTED);
        g2.drawString("Maks.", x + 25, top + 10);
        g2.setColor(LOW);
        g2.fillRoundRect(x + 82, top + 4, 18, 5, 5, 5);
        g2.setColor(AppTheme.MUTED);
        g2.drawString("Min.", x + 107, top + 10);
        g2.setColor(RAIN);
        g2.fillRoundRect(x + 150, top + 2, 13, 9, 5, 5);
        g2.setColor(AppTheme.MUTED);
        g2.drawString("Shi", x + 170, top + 10);
    }
    ...
    private void drawTemperatureLine(
        ...
        for (int i = 0; i < count; i++) {
            double value = high ? forecasts.get(i).highTemperature() : forecasts.get(i).lowTemperature();
            int x = xAt(i, count, left, right);
            int y = yAt(value, min, max, top, bottom);
            if (i == 0) {
                path.moveTo(x, y);
            } else {
                path.lineTo(x, y);
        ...
        Color color = high ? HIGH : LOW;
        g2.setStroke(new BasicStroke(high ? 3f : 2.4f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        g2.setColor(color);
        g2.draw(path);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (high && (i == 0 || i == count - 1 || i % 2 == 0)) {
                g2.drawString(format(value) + "°", x - 14, y - 9);
            }
        }
    }

    private void drawDateLabels(Graphics2D g2, int left, int right, int bottom) {
        g2.setFont(AppTheme.BODY);
        g2.setColor(AppTheme.MUTED);
        FontMetrics metrics = g2.getFontMetrics();
        int count = forecasts.size();
        for (int i = 0; i < count; i++) {
            String label = dateLabel(forecasts.get(i).date());
            int x = xAt(i, count, left, right);
